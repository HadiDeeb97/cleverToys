...
export const onRequest: MiddlewareHandler = async (context, next) => {
  ...
  // Replace the old default greeting ("I'm interested with your product") saved in older settings.
  const whatsappUrl = (() => {
    const raw = safeExternalUrl(storeSettings.whatsapp_url, fallbackWhatsappUrl);
    try {
      const url = new URL(raw);
      const text = url.searchParams.get('text');
      if (!text || /^hello,?\s*i'?m interested with your product\.?$/i.test(text.trim())) url.searchParams.set('text', 'Hello Clever Toys! I have a question about your toys.');
      return url.toString();
    } catch {
      return raw;
    }
  ...
  const storeScript = '<script src="/store-ui.js?v=20260924-5" defer></script><script src="/branding-ui.js?v=20260924-3" defer></script>';
  let output = html;
