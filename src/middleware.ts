...
export const onRequest: MiddlewareHandler = async (context, next) => {
  ...
  const requiredScript = `<script>(()=>{const mark=()=>document.querySelectorAll('label').forEach(label=>{if(label.querySelector('input:required,textarea:required,select:required')&&!label.querySelector('.required-star')){const s=document.createElement('span');s.className='required-label-text';s.textContent=(label.childNodes[0]?.textContent||'').trim();if(s.textContent){label.childNodes[0].textContent='';label.insertBefore(s,label.firstChild);const star=document.createElement('strong');star.className='required-star';star.textContent=' *';s.appendChild(star);}}});mark();new MutationObserver(mark).observe(document.documentElement,{childList:true,subtree:true});})();</script>`;
  const requiredStyles = `<style id="clever-required-styles">.required-label-text{display:block}.required-star{color:#e11d48;font-weight:900}.required-label-text+.required-star{display:inline}</style>`;
  ...
  if (!output.includes('/store-ui.js')) output = output.replace('</head>', `${earlyTheme}${storeConfig}${requiredStyles}${requiredScript}${storeScript}</head>`);
