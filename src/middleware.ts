...
export const onRequest: MiddlewareHandler = async (context, next) => {
  ...
  const storeScript = '<script src="/store-ui.js?v=20260915-globaltheme4" defer></script><script src="/branding-ui.js?v=20260918-1" defer></script>';
  ...
  if (!path.startsWith('/admin')) {
    const socialLinks = (showWhatsapp || showInstagram)
      ? `<div class="clever-topbar"><div class="container clever-topbar-inner"><span class="clever-topbar-brand">Clever Toys</span><nav aria-label="Store social links">${showWhatsapp ? `<a href="${escapeAttr(whatsappUrl)}" target="_blank" rel="noopener noreferrer" aria-label="Chat with Clever Toys on WhatsApp">WhatsApp</a>` : ''}${showInstagram ? `<a href="${escapeAttr(instagramUrl)}" target="_blank" rel="noopener noreferrer" aria-label="Clever Toys on Instagram">Instagram</a>` : ''}</nav></div></div>`
      : '';
    const ribbon = showRibbon
      ? `<div class="clever-ribbon" role="status"><div class="container">${escapeHtml(ribbonText)}</div></div>`
      : '';
    const socialStyles = `<style id="clever-branding-bar-styles">.clever-topbar{background:#111827;color:#fff;font-size:.78rem}.clever-topbar-inner{min-height:34px;display:flex;align-items:center;justify-content:space-between;gap:14px}.clever-topbar-brand{opacity:.72;font-weight:700}.clever-topbar nav{display:flex;align-items:center;gap:16px}.clever-topbar a{color:#fff;text-decoration:none;font-weight:800}.clever-topbar a:hover{text-decoration:underline}.clever-ribbon{background:var(--brand-primary,#111827);color:#fff;text-align:center;font-size:.82rem;font-weight:800}.clever-ribbon .container{min-height:38px;display:grid;place-items:center;padding-top:7px;padding-bottom:7px}@media(max-width:640px){.clever-topbar-brand{display:none}.clever-topbar-inner{justify-content:center}.clever-topbar nav{gap:18px}.clever-ribbon{font-size:.76rem}}</style>`;
    ...
    const controls = `<div class="clever-floating-controls" aria-hidden="false"><a id="clever-floating-cart" class="clever-floating-cart" href="/cart" title="View your cart" aria-label="Shopping cart"><span class="floating-cart-icon" aria-hidden="true">🛒</span><span class="floating-cart-count" aria-hidden="true" hidden>0</span></a><a id="clever-floating-whatsapp" class="clever-floating-whatsapp" href="${escapeAttr(whatsappUrl)}" target="_blank" rel="noopener noreferrer" aria-label="Chat with Clever Toys on WhatsApp" title="Chat with us on WhatsApp"><svg viewBox="0 0 32 32" focusable="false" aria-hidden="true"><path d="M16 3.2C9.1 3.2 3.5 8.8 3.5 15.7c0 2.2.6 4.4 1.8 6.3L3.2 28.8l6.9-2.1c1.8 1 3.8 1.5 5.9 1.5 6.9 0 12.5-5.6 12.5-12.5S22.9 3.2 16 3.2Zm0 22.8c-1.9 0-3.8-.5-5.4-1.5l1.2-4-.3-.4c-1-1.6-1.5-3.5-1.5-5.4-1.5-1.9-3.8-2.9-4.1-1.2l6.9-2.1c1.8 1 3.8 1.5 5.9 1.5 6.9 0 12.5-5.6 12.5-12.5S22.9 3.2 16 3.2Zm0 0c-1.9 0-3.8-0.5-5.4-1.5l1.2-4-.3-.4c-1-1.6-1.5-3.5-1.5-5.4 1.5-2.1 5.1-3.7 8.8-3.7 5.7 0 10.4 4.7 10.4 10.4S21.7 26.5 16 26.5Zm5.8-7.8c-.3-.2-1.8-.9-2.1-1-.3-.1-.5-.2-.7.2-.2.3-.8 1-.9 1.2-.2.2-.3.2-.6.1-.3-.2-1.3-.5-2.4-1.5-.9-.8-1.5-1.7-1.7-2-.2-.3 0-.5.1-.7.1-.1.3-.3.4-.5.1-.2.2-.3.3-.5.1-.2 0-.4 0-.6-.1-.2-.7-1.7-.9-2.3-.2-.6-.5-.5-.7-.5h-.6c-.2 0-.5.1-.8.4-.3.3-1 1-1 2.5s1 2.9 1.1 3.1c.1.2 2 3.1 4.9 4.3.7.2 1.4.2 1.9.1.6-.1 1.8-.7 2.1-1.4.3-.7.3-1.3.2-1.4-.1-.1-.3-.2-.6-.4Z" fill="currentColor"/></svg></a></div>`;
    if (!output.includes('clever-topbar')) {
      output = output.replace(/<header([^>]*class=[\"'][^\"']*site-header[^\"']*[\"'][^>]*)>/i, `${socialStyles}${socialLinks}${ribbon}$&`);
    }
