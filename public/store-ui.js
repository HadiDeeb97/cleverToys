(() => {
  ...
  const ensureFloatingStyles = () => {
    ...
    style.textContent = `
      ...
      .clever-floating-whatsapp{position:fixed;left:20px;bottom:20px;z-index:1002;width:58px;height:58px;border-radius:50%;display:flex;align-items:center;justify-content:center;background:#25D366;color:#fff;box-shadow:0 10px 26px rgba(15,23,42,.22);text-decoration:none;transition:transform .2s,box-shadow .2s}
      .clever-floating-whatsapp:hover{transform:translateY(-3px);box-shadow:0 14px 30px rgba(15,23,42,.27)}
      .clever-floating-whatsapp svg{width:30px;height:30px}
      @media(max-width:640px){
        .clever-floating-cart{right:14px;bottom:14px;width:54px;height:54px}
        .clever-floating-whatsapp{left:14px;bottom:14px;width:54px;height:54px}
        .clever-floating-cart .floating-cart-icon{font-size:23px}
        .clever-floating-whatsapp svg{width:28px;height:28px}
      }
    `;
  ...
  const updateFloatingCart = count => {
    const button = document.getElementById('clever-floating-cart');
    if (!button) return;
    const badge = button.querySelector('.floating-cart-count');
    if (badge) { badge.textContent=String(count); badge.hidden=count===0; }
    button.setAttribute('aria-label',count?`Shopping cart, ${count} ${count===1?'item':'items'}`:'Shopping cart');
  };
  ...
  const ensureFloatingCart = () => {
    ...
    if (!button) {
      ...
      button.id='clever-floating-cart';
      button.className='clever-floating-cart';
      button.href='/cart';
      button.title='View your cart';
      button.innerHTML='<span class="floating-cart-icon" aria-hidden="true">🛒</span><span class="floating-cart-count" aria-hidden="true"></span>';
  ...
  const ensureCartLink = () => {
    ...
    document.querySelectorAll('.main-nav').forEach(nav=>{
      ...
      if(links.length===0){const cart=document.createElement('a');cart.href='/cart';nav.appendChild(cart);normalizeCartLink(cart);} else if(links.length>1) links.slice(1).forEach(link=>link.remove());
    });
  ...
  const ensureWhatsAppBubble = () => {
    ...
    if(!bubble){
      ...
      bubble.className='whatsapp-bubble clever-floating-whatsapp';
      bubble.href=WHATSAPP_URL;
      bubble.target='_blank';
      bubble.rel='noopener noreferrer';
      bubble.setAttribute('aria-label','Chat with Clever Toys on WhatsApp');
      bubble.title='Chat with us on WhatsApp';
      bubble.innerHTML='<svg viewBox="0 0 32 32" focusable="false" aria-hidden="true"><path d="M16 3.2C9.1 3.2 3.5 8.8 3.5 15.7c0 2.2.6 4.4 1.8 6.3L3.2 28.8l6.9-2.1c1.8 1 3.8 1.5 5.9 1.5 6.9 0 12.5-5.6 12.5-12.5S22.9 3.2 16 3.2Zm0 22.8c-1.9 0-3.8-.5-5.4-1.5l-.4-.2-4.1 1.2 1.2-4-.3-.4c-1-1.6-1.5-3.5-1.5-5.4C5.5 9.9 10.2 5.2 16 5.2s10.5 4.7 10.5 10.5S21.8 26 16 26Zm5.8-7.8c-.3-.2-1.8-.9-2.1-1-.3-.1-.5-.2-.7.2-.2.3-.8 1-.9 1.2-.2.2-.3.2-.6.1-.3-.2-1.3-.5-2.4-1.5-.9-.8-1.5-1.7-1.7-2-.2-.3 0-.5.1-.7.1-.1.3-.3.4-.5.1-.2.2-.3.3-.5.1-.2 0-.4 0-.6-.1-.2-.7-1.7-.9-2.3-.2-.6-.5-.5-.7-.5h-.6c-.2 0-.5.1-.8.4-.3.3-1 1-1 2.5s1 2.9 1.1 3.1c.1.2 2 3.1 4.9 4.3.7.3 1.3.5 1.7.7.6-.1 1.8-.7 2.1-1.4.3-.7.3-1.3.2-1.4-.1-.1-.3-.2-.6-.4Z" fill="currentColor"/></svg>';
      document.body.appendChild(bubble);
    } else {
      bubble.classList.add('clever-floating-whatsapp');
      bubble.href=WHATSAPP_URL;
    }
  };
  ...
  const patchStorage=()=>{try{if(localStorage.__cleverPatched)return;const originalSet=localStorage.setItem.bind(localStorage),originalRemove=localStorage.removeItem.bind(localStorage);localStorage.__cleverPatched=true;localStorage.setItem=(key,value)=>{originalSet(key,value);if(key===CART_KEY)window.dispatchEvent(new CustomEvent('clever-cart-updated'));};localStorage.removeItem=key=>{originalRemove(key);if(key===CART_KEY)window.dispatchEvent(new CustomEvent('clever-cart-updated'));};}catch{}};
  const refreshFloating=()=>{ensureFloatingCart();ensureWhatsAppBubble();};
  const init=()=>{applyCachedThemeImmediately();ensureStoreHeader();patchLogos();ensureCartLink();refreshFloating();if(FALLBACK_LOGO)extractLogoTheme(FALLBACK_LOGO);};
  window.addEventListener('storage',e=>{if(e.key===CART_KEY){ensureCartLink();updateFloatingCart(cartCount());}});window.addEventListener('clever-cart-updated',()=>{ensureCartLink();updateFloatingCart(cartCount());});window.addEventListener('cart-updated',()=>{ensureCartLink();updateFloatingCart(cartCount());});
  document.addEventListener('DOMContentLoaded',()=>{ensureStoreHeader();patchLogos();ensureCartLink();refreshFloating();});
  window.addEventListener('pageshow',refreshFloating);
  patchStorage();init();
})();