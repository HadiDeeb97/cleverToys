(() => {
  ...
  const updateCartLinks = () => { const count=cartCount(); document.querySelectorAll('a[href="/cart"],a[href="/cart/"]').forEach(link=>{normalizeCartLink(link);const badge=link.querySelector('.cart-count-badge');if(badge){badge.textContent=String(count);badge.hidden=count===0;}link.setAttribute('aria-label',count?`Shopping cart, ${count} ${count===1?'item':'items'}`:'Shopping cart');}); };
  const ensureCartLink = () => { if(location.pathname.startsWith('/admin'))return; document.querySelectorAll('.main-nav').forEach(nav=>{const links=[...nav.querySelectorAll('a[href="/cart"],a[href="/cart/"]')];if(links.length===0){const cart=document.createElement('a');cart.href='/cart';nav.appendChild(cart);normalizeCartLink(cart);}else if(links.length>1)links.slice(1).forEach(link=>link.remove());});updateCartLinks(); };
  ...
  const ensureWhatsAppBubble = () => {
    ...
    bubble.className='whatsapp-bubble'; bubble.href=WHATSAPP_URL; bubble.target='_blank'; bubble.rel='noopener noreferrer'; bubble.setAttribute('aria-label','Chat with Clever Toys on WhatsApp'); bubble.title='Chat with us on WhatsApp';
    bubble.innerHTML='<span class="whatsapp-symbol" aria-hidden="true">☏</span>';
  ...
  const init=()=>{applyCachedThemeImmediately();ensureStoreHeader();patchLogos();ensureCartLink();ensureWhatsAppBubble();if(FALLBACK_LOGO)extractLogoTheme(FALLBACK_LOGO);};
  window.addEventListener('storage',e=>{if(e.key===CART_KEY)updateCartLinks();});window.addEventListener('clever-cart-updated',updateCartLinks);window.addEventListener('cart-updated',updateCartLinks);
  document.addEventListener('DOMContentLoaded',()=>{ensureStoreHeader();patchLogos();ensureCartLink();ensureWhatsAppBubble();});window.addEventListener('pageshow',()=>{ensureStoreHeader();patchLogos();ensureCartLink();ensureWhatsAppBubble();});
  patchStorage();init();
})();